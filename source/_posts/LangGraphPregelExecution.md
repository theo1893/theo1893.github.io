...
在前面的章节我们介绍过, Pregel节点负责处理输入的数据并输出. 实际实现比这个要复杂. 在运行时, Pregel节点的可执行对象实际上由下面2个部分构成: 

1. 用户定义的Action, 比如函数体.
2. 更新Task运行时数据, 负责将Action的返回值临时存储到Task, 并且向下游节点的触发通道写入数据, 等待全部任务结束后由上层循环统一处理.

Pregel节点在运行时被构造为Pregel任务, 任务具有运行时的临时数据, 这里提到的第2步, 即为更新任务的运行时数据. 以此处的代码为例, node1对应的Pregel任务的示例图大致如下图所示:
